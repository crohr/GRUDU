...
public class JCTerm  extends JPanel implements KeyListener, ActionListener, Runnable, Term{
	...
	public JCTerm(HashMap<String, String> properties){
	...
	public void run(){
		...
		while(thread!=null){
			try{
				int port=22;
				try{
					String a =  myProperties.get("username");
					String b = myProperties.get("host");
					String c = myProperties.get("publickey");
					user = a;
					host = b;
//					user = "dloureiro";
//					host = "acces.lyon.grid5000.fr";
					port = 22;
					//jsch.addIdentity("/home/david/.ssh/id_dsa");
					jsch.addIdentity(c);
				}
				catch(Exception ee){
					ee.printStackTrace();
				...
				if(mode==SHELL){
					...
					channel.connect();
					String destination = myProperties.get("destination");
					if(!destination.equalsIgnoreCase("")){
						out.write(("ssh "+destination+"\n").getBytes());
						out.flush();
						clear();
						String command = myProperties.get("command");
						if(command != null){
							out.write((command+"\n").getBytes());
							out.flush();
							clear();
...
//					out.write(emulator.getCodeENTER(),0,emulator.getCodeENTER().length);
//					out.flush();
				}
				else if(mode==SFTP){
...
//					in=channel.getInputStream();
				...
				emulator=new EmulatorVT100(this, in,JCTerm.this);
				emulator.reset();
				emulator.start();
			}
			catch(Exception e){
...
//			JOptionPane.OK_CANCEL_OPTION);
//			if(result==JOptionPane.OK_OPTION){
//			passphrase=pword.getText();
//			return true;
//			}
//			else{ return false; }
			passphrase = myProperties.get("passphrase");
			return true;
		}
...
//		mb.add(m);
	...
	public static void newTerminal(HashMap<String, String> myProperties){
		final JCTerm term=new JCTerm(myProperties);
	...
	public static void main(String[] args){
		...
		HashMap<String, String> myProperties = new HashMap();

		JCTerm term=new JCTerm(myProperties);
		JFrame frame=new JFrame("JCTerm");
...
//		term.kick();
	...
	public void quit(){
		try{
			...
			((JFrame)term_area.getParent().getParent().getParent()).dispose();
		}
		catch(Exception e){
			e.printStackTrace();
			((JFrame)term_area.getParent().getParent().getParent()).dispose();
